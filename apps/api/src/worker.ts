...
import { holds, jobs, seats, showSeats, waitlistEntries } from './db/schema';
...
async function handleJob(job: typeof jobs.$inferSelect) {
  if (job.type === 'release_expired_holds') {
    await db.transaction(async tx => {
      const expired = await tx.select({ id: holds.id, showId: holds.showId, seatIds: holds.seatIds }).from(holds).where(and(eq(holds.status, 'active'), sql`${holds.heldUntil} <= now()`));
      ...
      for (const hold of expired) await allocateWaitlist(tx, hold.showId, Array.isArray(hold.seatIds) ? hold.seatIds as string[] : []);
    });
  }
  if (job.type === 'allocate_waitlist') {
    const payload = job.payload as {showId?: string; seatIds?: string[]};
    if (payload.showId && payload.seatIds?.length) await db.transaction(tx => allocateWaitlist(tx, payload.showId!, payload.seatIds!));
  }
...
async function allocateWaitlist(tx: any, showId: string, seatIds: string[]) {
  for (const showSeatId of seatIds) {
    const seat = (await tx.select({category: seats.category}).from(showSeats).innerJoin(seats,eq(seats.id,showSeats.seatId)).where(and(eq(showSeats.id,showSeatId),eq(showSeats.showId,showId),eq(showSeats.status,'available'))).limit(1))[0];
    if (!seat) continue;
    const next = (await tx.select({id: waitlistEntries.id, userId: waitlistEntries.userId}).from(waitlistEntries).where(and(eq(waitlistEntries.showId, showId), eq(waitlistEntries.category, seat.category), eq(waitlistEntries.status, 'waiting'))).orderBy(asc(waitlistEntries.createdAt)).for('update', { skipLocked: true }).limit(1))[0];
    if (!next) continue;
    const until = new Date(Date.now() + 15 * 60_000);
    const updated = await tx.update(showSeats).set({status:'held',heldByUserId:next.userId,heldUntil:until,version:sql`${showSeats.version}+1`}).where(and(eq(showSeats.id,showSeatId),eq(showSeats.status,'available'))).returning({id:showSeats.id});
    if (!updated[0]) continue;
    await tx.insert(holds).values({userId:next.userId,showId,seatIds:[showSeatId],heldUntil:until});
    await tx.update(waitlistEntries).set({status:'offered',offeredSeatIds:[showSeatId],offeredAt:new Date(),offerExpiresAt:until}).where(eq(waitlistEntries.id,next.id));
  }
}
...
function startBullMqWorker() {
  ...
  const worker = new Worker('encore-jobs', async bullJob => {
    ...
    try {
      ...
      if (row.type === 'release_expired_holds') await db.insert(jobs).values({ type: 'release_expired_holds', payload: {}, availableAt: new Date(Date.now() + 30_000) });
    } catch (error) {
      const exhausted = bullJob.attemptsMade + 1 >= 5;
      await db.update(jobs).set({ status: exhausted ? 'failed' : 'pending', lastError: error instanceof Error ? error.message.slice(0, 500) : 'Job failed', lockedAt: null, lockedBy: null }).where(eq(jobs.id, row.id));
      throw error;
    }
  ...
  const enqueue = async () => {
    const pending = await db.select().from(jobs).where(and(eq(jobs.status, 'pending'), lte(jobs.availableAt, new Date()))).limit(50);
    for (const job of pending) await queue.add(job.type, { dbJobId: job.id }, { jobId: job.id, attempts: 5, backoff: { type: 'exponential', delay: 1000 }, removeOnComplete: 1000, removeOnFail: 5000 });
  };
  void enqueue();
