...
export function SeatPicker({ eventId }: { eventId: string }) {
  ...
  const isConcert =
    eventMeta?.kind?.toLowerCase() === 'concert' ||
    matchedStatic?.kind?.toLowerCase() === 'concert' ||
    (typeof eventMeta?.title === 'string' && eventMeta.title.toLowerCase().includes('concert')) ||
    (typeof matchedStatic?.title === 'string' && matchedStatic.title.toLowerCase().includes('concert'));
