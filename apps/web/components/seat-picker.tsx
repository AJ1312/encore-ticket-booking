...
type Seat = { id: string; row: string; number: number; pricePaise: number; status: SeatStatus };
...
export function SeatPicker({ eventId = 'the-night-we-remember' }: { eventId?: string }) {
  ...
  return <main className="booking-page"><PortalNav /><section className="booking-head"><Link href="/events" className="back-link"><ArrowLeft size={15}/> Back to guide</Link><div className="booking-title-row"><div><span className="eyebrow">{event.title} · {event.venue}</span><h1>Choose your<br/><em>seats.</em></h1><p>{event.date} 2026 · {event.time} · {event.city}</p></div><div className="hold-note"><Clock3 size={15}/> 10 minute hold</div></div></section><section className="booking-area"><div className="map-column">{loading ? <div className="empty-state">Loading live seat inventory…</div> : error ? <div className="empty-state"><h3>Seats unavailable</h3><p>{error}</p></div> : <><div className="map-toolbar"><div className="map-tabs"><button className={view === 'map' ? 'active' : ''} onClick={() => setView('map')}>Map view</button><button className={view === 'list' ? 'active' : ''} onClick={() => setView('list')}>List view</button></div><div className="zoom-button"><button aria-label="Zoom out" onClick={() => setZoom(value => Math.max(80, value - 10))}><Minus size={14}/></button><span>{zoom}%</span><button aria-label="Zoom in" onClick={() => setZoom(value => Math.min(120, value + 10))}><Plus size={14}/></button></div></div>{view === 'map' ? <div className="seat-canvas"><div className="seat-map-scale" style={{ transform: `scale(${zoom / 100})` }}><div className="stage">STAGE</div><div className="seat-grid-large">{seats.map(seat => <button key={seat.id} disabled={seat.status !== 'available'} aria-label={`Row ${seat.row}, seat ${seat.number}`} onClick={() => toggle(seat.id)} className={`seat-large ${seat.status === 'booked' ? 'sold' : seat.status} ${selected.includes(seat.id) ? 'selected' : ''}`}>{seat.number}</button>)}</div></div><div className="seat-legend"><span><i/> Available</span><span><i className="selected-dot"/> Selected</span><span><i className="sold-dot"/> Unavailable</span></div><p className="seat-helper"><Info size={14}/> Select up to 8 seats. Prices include all taxes.</p></div> : <div className="seat-list-view">{seats.map(seat => <button key={seat.id} disabled={seat.status !== 'available'} onClick={() => toggle(seat.id)} className={`seat-list-item ${selected.includes(seat.id) ? 'selected' : ''}`}><span>Row {seat.row} · Seat {seat.number}</span><b>{seat.status !== 'available' ? 'Unavailable' : `₹${Math.round(seat.pricePaise / 100).toLocaleString('en-IN')}`}</b></button>)}</div>}</>}</div><aside className="booking-summary"><span className="eyebrow">Your evening</span><h2>{event.title}</h2><p className="summary-meta">{event.venue}, {event.city}<br/>{event.date} 2026 · {event.time}</p><div className="summary-rule"/><div className="summary-seats"><div><span>Selected seats</span><b>{selected.length ? selected.map(id => { const seat = seats.find(value => value.id === id); return seat ? `${seat.row}${seat.number}` : id; }).join(', ') : 'None yet'}</b></div><div><span>Ticket total</span><b>₹{Math.round(total / 100).toLocaleString('en-IN')}</b></div></div><button onClick={continueToCheckout} className={`coral-button summary-cta ${selected.length ? '' : 'disabled'}`}><Check size={16}/> {selected.length ? 'Continue to checkout' : 'Select a seat'} <ChevronRight size={16}/></button><p className="summary-foot">You’ll sign in before checkout so your ticket stays with you.</p></aside></section><PortalFooter/></main>;
}
